...
int main(int argc, char **argv)
{
	...
	const char * fontname = HPDF_LoadTTFontFromFile(pdf, "C:\\Windows\\Fonts\\arial.ttf", HPDF_TRUE);
	//const char * fontname = HPDF_LoadTTFontFromFile(pdf, "Courie", HPDF_TRUE);
	...
	HPDF_Page_ShowText(page, "JpegDemo Хрень");
	//HPDF_Page_EndText(page);

	HPDF_Page_TextOut(page, 40.0, 782.0,
		"€ Текст на русском языке в кодировке CP1251");
	HPDF_Page_TextRect(page, 0, 200, 782, 0,
		"€ Текст на русском языке в кодировке CP1251",
		HPDF_TALIGN_LEFT, 0);
	...
	draw_image(pdf, "rgb.jpg", 70, HPDF_Page_GetHeight(page) - 410,	 "24bit color image (Хрень так, так)");
