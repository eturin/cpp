...
char * create_env_isapi(const struct Client * pcln, const struct Worker * pwrk) {
	...
	sprintf(pTemp, "ISAPI_PATH=%s", pcln->psrv->work_path);	
	pTemp += strlen(pTemp) + 1;

	sprintf(pTemp, "ISAPI_PORT=%d", pcln->psrv->iport);
	pTemp += strlen(pTemp) + 1;
...
struct Worker * init_Worker(const char *name, size_t len, struct Client * pcln, LPVOID iocp) {
	...
	while(isOk) {
		...
		if(strstr(pwrk->name, "isapi") != NULL) {
			...
			pwrk->addr.sin_port = htons(pcln->psrv->iport);
	...
	if(pwrk != NULL) {
		...
		if(pwrk->type == WORKER_ISAPI)
			...
			do {
				...
				memcpy(&addr, &pwrk->addr, sizeof(struct sockaddr_in));
				if(0 != WSAConnect(sfd, (const struct sockaddr*)&addr, sizeof(struct sockaddr_in), NULL, NULL, NULL, NULL)) {
					/*worker isapi лежит*/
					break;
				}

				/*отправляем команду принудительной остановки*/
				WSABUF buf;
				buf.buf = "OFF";
				buf.len = 3;
				DWORD len = 0;
				...
				Sleep(2000);//подождем выключения
			} while(FALSE);
