...
enum ENUMVAR {
	VTYPE_EMPTY = 0,
	VTYPE_NULL  = 1,
	VTYPE_I2    = 2,            //для короткого целого (short) int16_t 
	VTYPE_I4    = 3,            //для целого числа (int) int32_t
	VTYPE_R4    = 4,            //для числа с плавающей точкой  float
	VTYPE_R8    = 5,            //для числа с плавающей точкой  double 
	VTYPE_DATE  = 6,            //DATE (double)
	VTYPE_TM    = 7,            //struct tm
	VTYPE_PSTR  = 8,            //для строки (char*) struct str    string
	VTYPE_INTERFACE = 9,        //struct iface
	VTYPE_ERROR =10,            //int32_t errCode
	VTYPE_BOOL  =11,            //для bool
	...
	VTYPE_PWSTR =22,            //для строки (WCHAR_T*) struct wstr
...
struct _tVariant {
	_ANONYMOUS_UNION union {
		...
		_ANONYMOUS_STRUCT struct {
			char*        pstrVal;//для строки char*
			uint32_t     strLen; //count of bytes
		} __VARIANT_NAME_3/*str*/;
		_ANONYMOUS_STRUCT struct {
			WCHAR_T*    pwstrVal;//для строки WCHAR_T*
