...
bool Base::Init(void* pConnection) {
	...
	pAdapter->SetEventBufferDepth(100);
...
/*"Рефлексия" из 1С*/
long Base::FindProp(const WCHAR_T* ex_name) const {
	long res = -1;
...
bool Base::IsPropReadable(const long num) const {
	if(num < cnt_props)
		return Props[num]->r;
	else
		return false;
}
bool Base::IsPropWritable(const long num) const {
	if(num < cnt_props)
		return Props[num]->w;
	else
		return false;
}
const WCHAR_T* Base::GetPropName(long num, long cur_locale) const {
	if(num < cnt_props) {
		wchar_t * temp;
		if(cur_locale == 0)
			temp=Props[num]->en;
...
long Base::FindMethod(const WCHAR_T* ex_name) const {
	long res = -1;
	wchar_t * temp = nullptr;
	WCHAR_to_wchar(temp, ex_name);
...
bool Base::HasRetVal(const long num) const {
	if(num < cnt_methods)
		return Methods[num]->is_proc == false;
	else
		return false;
}
const WCHAR_T* Base::GetMethodName(const long num, const long cur_locale) const {
	if(num < cnt_methods) {
		wchar_t * temp;
		if(cur_locale == 0)
			temp = Methods[num]->en;
...
long Base::GetNParams(const long num) const {
	if(num < cnt_methods)
...
void Base::addError(uint32_t wcode, const wchar_t* source, const wchar_t* descriptor, long code) const {
	if(pAdapter) {
		WCHAR_T *err   = nullptr;
...
void Base::addError(const wchar_t * msg) const {
	int      no = GetLastError();
	...
	if(FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, no, MAKELANGID(LANG_RUSSIAN, SUBLANG_DEFAULT), str_err, sizeof(str_err), NULL))
		wsprintf(str_err, L"%s:\n%s\n", msg, str_err);
	else
		wsprintf(str_err, L"%s:\nномер ошибки %d\n", msg, no);
	//pAdapter->ExternalEvent(this->name, L"Error", str_err);		
	pAdapter->AddError(no, this->name, str_err, no);
}

void Base::addMsg(wchar_t * str_type, wchar_t * msg) const {
	pAdapter->ExternalEvent(this->name, str_type, msg);
}