...
/*ПРОЧИЕ МЕТОДЫ*/
/*конвертация char* -> wchar_t*/
size_t char_to_wchar(wchar_t* &Dest, const char* Source, size_t len) {
	if(len == 0) {//если размер задан, то и место уже зарезервировано
		len = std::strlen(Source)+1;
		Dest = new wchar_t[len];
	}	
	return mbstowcs(Dest, Source, len);
}
/*Конвертация wchar_t* -> WCHAR_T* (на ведре WCHAR_T==wchar_t)*/
...
static const wchar_t Class_Names[] = L"Criterion|KeyHook"; //|OtherClass1|OtherClass2
...
long GetClassObject(const WCHAR_T* ex_name, Base** pInterface) {
	if(!*pInterface) {
		...
		if(!wcscmp(name, L"Criterion"))
			*pInterface = new Criterion();
		else if(!wcscmp(name, L"KeyHook"))
			*pInterface = new KeyHook();
		delete[] name;
